...
def gen_burst_spike_times(spike_opts):
...
## Parse inputs
    ...
    if np.size(spike_opts.rate) == 1:
        if spike_opts.rate_dist == 'uniform':   
            spike_opts.rate = spike_opts.rate*np.ones((spike_opts.K,1))       # Sample each neuron's firing rate from a Gamma distribution
        elif spike_opts.rate_dist == 'gamma':
            rate_tmp = np.random.gamma(spike_opts.alpha,spike_opts.rate,
    ...
    S = np.zeros((spike_opts.K, spike_opts.nt))                                    # Initialize spike matrix

    for kk in range(spike_opts.K):
        T_tot = 0                                                             # Initialize the time counter to zero
        while T_tot<spike_opts.nt:
            t_arr = -1*spike_opts.rate(kk)*np.log(np.random.rand(1))                       # Take an independent exponential step (def of Poisson process)
            S[kk,min(np.ceil(T_tot+t_arr),spike_opts.nt+1)] = 1                  # Set a spike down at that location - If the spike is outside the [1,nt] range, set it to be at nt+1 so that a huge matrix is not generated
            ...
            if spike_opts.burst_mean > 0:                                       # Check if bursting behavior is warrented given the mean number of spikes per burst (spike_opts.burst_mean)
                num_in_burst = 1 + np.ranodm.poisson(spike_opts.burst_mean)            # Get number of spikes in the burst
                if num_in_burst > 1:
                    for ll in np.linspace(2,num_in_burst):
                        t_arr = ref_time + 2*np.random.rand(1)                          # Jitter the actual times of each spike in the burst
                        S[kk,min(np.ceil(T_tot+t_arr),spike_opts.nt+1)] = 1      # Set a spike down at that location - If the spike is outside the [1,nt] range, set it to be at nt+1 so that a huge matrix is not generated
                        T_tot = T_tot + t_arr                                 # Keep counting time (increment time counter)

...
def growCapillaries(nodes,conn,neur_ves,nv,vp,vres):
    ...
    capppos  = capppos*vres+1-randi(max(1,vres),size(capppos));
    ...
    while(lflag)
      ...
      if(min(cappsum((nv.nvert_sum+1):))>1)
        lflag = 0;
      else
        [!,idxs] = find(cappsum==1);
    ...
    for i = 1:length(connToConnect)
        connMat(conn(connToConnect(i)).s,conn(connToConnect(i)).start) = connToConnect(i);
      ...
      if(isnan(conn(currConn).weight))
        ...
        if(sum(isnan(Weights)))
          ...
          if(length(Weights)==1)
            ...
            TMP1    = (max(Weights)^2)-(min(Weights)^2);
            TMP2    = (max(Weights)^2)+(min(Weights)^2);
        ...
        toConnect = [toConnect;Conns(isnan(Weights))'; startConns(isnan(startWeights))'];
      
      toConnect = toConnect(2:);

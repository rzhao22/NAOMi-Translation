...
#                   samples/um)
#       .N_den    - Width of apical drites (default = 10)
...
#                 neuron. THis array will be augmented with drite values
...
def generateNeuralVolume(neur_params,vol_params,neur_locs,Vcell,Vnuc,neur_ves):
    ...
    neur_soma = np.zeros(vol_sz*vres, 'uint16');                                 # Initialize the array that stores where all the different somas are
    neur_vol  = np.zeros(vol_sz*vres, 'single');                                 # Initialize the full neural voume array
    gp_nuc    = dict(vol_params.N_neur,2);                                     # Set up a cell array to hold the GPs for the nuclei
    gp_soma   = dict(vol_params.N_neur,1);                                     # Set up a cell array to hold the GPs for the nuclei
    ...
    taken_pts = taken_pts(range(len(taken_pts)),range(len(taken_pts)),range(vol_depth+1,vol_depth+vol_sz(3)*vres));# Only keep the portion of the vasculature volume that intersects with the fully simulated volume
    ...
    t = TicToc()
    for kk in range(1,vol_params.N_neur):
        t.tic()
        max_ext = dict(max(sqrt(sum(bsxfun(@plus,Vcell(:,:,kk),...
                                                    -neur_locs(kk,:)).^2,2)))); # Find the furthest point of the soma from the "origin"
        mExtRes = dict(max_ext*vres);                                           # Round the value for integer indices below
        idx_pos = [round(vres*neur_locs(kk,1)), round(vres*neur_locs(kk,2)), round(vres*neur_locs(kk,3))];
        idxX    = max(1,idx_pos(1)-mExtRes):min(idx_pos(1)+mExtRes,vol_sz(1)*vres);
        idxY    = max(1,idx_pos(2)-mExtRes):min(idx_pos(2)+mExtRes,vol_sz(2)*vres);
        idxZ    = max(1,idx_pos(3)-mExtRes):min(idx_pos(3)+mExtRes,vol_sz(3)*vres);

        [mesh_x, mesh_y, mesh_z] = np.meshgrid(np.single((idxX-idx_pos(1))/vres), np.single((idxY-idx_pos(2))/vres), np.single((idxZ-idx_pos(3))/vres));
        mesh_x = permute(mesh_x,[2 1 3]);
        mesh_y = permute(mesh_y,[2 1 3]);
        mesh_z = permute(mesh_z,[2 1 3]);
        ...
        idx_tri = [mesh_x(idx_to_test)+idx_pos(1)/vres+1/vres/2, mesh_y(idx_to_test)+idx_pos(2)/vres+1/vres/2, mesh_z(idx_to_test)+idx_pos(3)/vres+1/vres/2];
    ...
    #                  mesh_y(idx_to_test),mesh_z(idx_to_test)]);                # Find the points inside the K^th nucleus to remove (no fluorescence)
        ...
        neur_soma(neur_idx2)   = int(kk);                                   # Number where the kth neuron is saved to the points inside the soma
        gp_soma{kk,1}          = int(neur_idx2);                             # Find points in the nucleus
        ...
        iZ = iZ+idxZ(1)-1;
        neur_idx2              = sub2ind(vol_sz*vres,iX,iY,iZ);
        gp_nuc{kk,1}           = int(neur_idx2);                   # Find points in the nucleus

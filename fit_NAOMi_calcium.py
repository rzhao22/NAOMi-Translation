def fit_NAOMi_calcium(spike_time, t_frame, fmean_norm, t_ephys, varargin):
    cal_params = {}
    if nargin > 4 :
        optType = varargin{1}
    else ：
        optType = 'fmincon'                                        # 'fmincon' or 'BayesOpt': default is 'fmincon'

    if nargin > 5 :
        nRestart = varargin{2}
    else:
        nRestart = 1000                                            # How many times to restart the parameter optimization

    if nargin > 6 :
        protType = varargin{3}
    else :
        protType = 'GCaMP6'                                        # What indicator to train for
    ...
    simFluor  = calcium_dynamics(spikes2, cal_params, protType)[2]         #  Simulate the fluorescence time-traces

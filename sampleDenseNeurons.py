...
def sampleDenseNeurons(neur_params,vol_params,neur_ves):
...
## Sample neural soma shapes and locations
    ...
    x, y, z = np.mgrid[-np.ceil(vol_params.min_dist/2):np.ceil(vol_params.min_dist/2)+1,
                   ...
                   -np.ceil(vol_params.min_dist/2):np.ceil(vol_params,min_dist/2)+1] # Set up a 3D grid to sample over
    se      = strel(sqrt(x.^2 + y.^2 + z.^2) <= vol_params.min_dist/2);        # SKIPPED Find all the points that are within the minimum distance to the above points
...
return [neur_locs,Vcell,Vnuc,Tri,rotAng]

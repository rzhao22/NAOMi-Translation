...
def check_dend_params(dend_params):
...
## Run the checks 
    ...
    if not(bool(dend_params)):
        dend_params = {}

    dParams ={}
    
    # dendritic tree number,radius in um of branches (uniform across circle),radius in um of branches (uniform in z)
    dParams.dtParams = [40, 150, 50, 1, 10]

    # apical dendrite (L2/3) number,radius in um of branches (uniform across circle),radius in um of branches (uniform in z),offset from center in um
    dParams.atParams = [6, 5, 5, 5, 1]

    # apical dendrite (L5) number,radius in um of branches (uniform across circle),radius in um of branches (uniform in z),offset from center in um
    dParams.atParams2 = [1, 5, 5, 5, 4]

    # weight for path planning randomness
    dParams.dweight = 10

    # weight for obstruction
    dParams.bweight = 5
    
    # scaling for dendrite thickness (int). Should be 1 for 1um sampling,(4 for 0.5um sampling)
    dParams.thicknessScale = 0.5

    # scaling for dendrite fluorescence (1/distance scaling [um], distance scaling weight [0-1], variation in fluoresence (uniform) [0-1])
    dParams.weightScale = [150, 1, 0.8]

    # dims set at 10 um per space
    dParams.dims = [60, 60, 60]

    # dims subsampling factor (10 samples per dim grid)
    dParams.dimsSS = [5, 5, 5]

    # Rall exponent that controls the cahnge in size of dendrites at branching locations (default = 1.5) 
    dParams.rallexp = 1.5

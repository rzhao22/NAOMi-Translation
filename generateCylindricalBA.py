...
def generateCylindricalBA(vol_params,psf_params):
  
  if (!isfield(vol_params,'vasc_sz')) | isempty(vol_params.vasc_sz):
    vol_params.vasc_sz = gaussianBeamSize(psf_params,vol_params.vol_depth+vol_params.vol_sz(3)/2)+vol_params.vol_sz+[0,0,1]*vol_params.vol_depth;# Get beam size
  
  cylscale = psf_params.cylscale
  fl       = np.single(psf_params.obj_fl/1000)                                 # focal length [m]
  D2       = np.single(1e-6*(1/vol_params.vres)/psf_params.ss)                 # observation grid spacing [m]
  N        = np.single(1e-6*(vol_params.vasc_sz(1:2)-vol_params.vol_sz(1:2))/D2)# gridsize
  D1       = np.single(max(gaussianBeamSize(psf_params,fl*1e6)/1e6)/min(N))    # source grid spacing [m]
  nre      = np.single(psf_params.n)                                           # immersion numerical index
  rad      = np.single(np.tan(np.asin(psf_params.NA/nre))*fl)                        # source radius [m]
  objrad   = np.single(np.tan(np.asin(psf_params.objNA/nre))*fl)                     # source radius [m]
  k        = 2*nre*np.pi/np.single(psf_params.lambdaWave*1e-6)                        # optical wavenumber [rad/m]
  [X,Y]    = np.meshgrid((-N(1)/2:N(1)/2-1)*D1,(-N(2)/2:N(2)/2-1)*D1)
  Uout     = generateGaussianProfile(X/cylscale(1),Y/cylscale(2),rad,objrad,k,fl)                       # generate gaussian wavefront with apodization 1

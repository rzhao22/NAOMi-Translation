...
###########################################################################
    ...
    while(i<=nsamps):
        ...
        if((pdf(rndpt[0],rndpt[1],rndpt[2])-np.random.rand)>0):
            xc = int32([max(0,2*width+1-rndpt[0]), min(0,sz[0]-rndpt[0]-2*width)+4*width]+1)
            yc = int32([max(0,2*width+1-rndpt[1]), min(0,sz[1]-rndpt[1]-2*width)+4*width]+1)
            zc = int32([max(0,2*width+1-rndpt[2]), min(0,sz[2]-rndpt[2]-2*width)+4*width]+1)
            
            xi = int32([max(1,rndpt[0]-2*width), min(sz[0],rndpt[0]+2*width)])
            yi = int32([max(1,rndpt[1]-2*width), min(sz[1],rndpt[1]+2*width)])
            zi = int32([max(1,rndpt[2]-2*width), min(sz[2],rndpt[2]+2*width)])

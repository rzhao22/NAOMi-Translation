...
def simulate_optical_propagation2(vol_params=None,psf_params=None,vol_out=None,scattvol=None):
  ...
  if (isfield(vol_out,'neur_ves_all'))& !isempty(vol_out.neur_ves_all):
    nvpx = len(vol_out.neur_ves_all);
    vcpx = vol_params.vasc_sz*vres;
    if(~all(nvpx==vcpx)):
      TMPvasc = False(vcpx);
      if(all(nvpx<=vcpx)):
        dpx = (vcpx-nvpx)/2;
        TMPvasc(1+floor(dpx(1)):-ceil(dpx(1)),1+floor(dpx(2)):-ceil(dpx(2)),
          range(1,-dpx(3)*2) == vol_out.neur_ves_all)
      elif(all(nvpx>=vcpx)):
        dpx = (nvpx-vcpx)/2;
        TMPvasc = vol_out.neur_ves_all(1+floor(dpx(1)):-ceil(dpx(1)),
          1+floor(dpx(2)):-1 * np.ceil(dpx(2)), 1:-2*dpx(3));
      else:
        dpx = (vcpx-nvpx)/2;
        if(vcpx(3)>=nvpx(3)):
          TMPvasc(1+floor(dpx(1)):-ceil(dpx(1)),1+floor(dpx(2)):-ceil(dpx(2)),
            1:-2*dpx(3)) = vol_out.neur_ves_all;
        else:
          TMPvasc(1+floor(dpx(1)):-ceil(dpx(1)),1+floor(dpx(2)):-ceil(dpx(2)),:)
            = vol_out.neur_ves_all(:,:,1:vcpx(3));

  #       nvpx = size(vol_out.neur_ves)/vres;
  #       dpx = (vcpx-nvpx)/2;
  #       TMPvasc(1+floor(dpx(1)):-ceil(dpx(1)),1+floor(dpx(2)):-ceil(dpx(2)), ...
  #         1:-2*dpx(3)) = vol_out.neur_ves;
  ...
  if(acc_flag)
    ...
    elseif strcmp(psf_params.type,'vtwins')
      Uin = generateVtwinsBA(vol_params,psf_params);
    elseif strcmp(psf_params.type,'vtwinsL')
      Uin = generateVtwinsBA(vol_params,psf_params);
      Uin = Uin.left;
    elseif strcmp(psf_params.type,'vtwinsR')
      Uin = generateVtwinsBA(vol_params,psf_params);
      Uin = Uin.right;
    elseif strcmp(psf_params.type,'bessel')
      Uin = generateBesselBA(vol_params,psf_params);
    else
    ...
    # Optical propagation
    if ~isstruct(Uin)
      [mask,psf] = genCorticalLightPath(vol_params,psf_params,vol,Uin);        # Generate diffraction-limited tissue obscuration mask
      ...
      for i = 1:length(names)
        [mask,psf]   = genCorticalLightPath(vol_params,psf_params,vol,...
  ...
  if (isfield(psf_params,'taillength'))&&(~isempty(psf_params.taillength))
    zC = zB+psf_params.taillength*vres;
  ...
  if (isfield(psf_params,'scatter_sz'))&&(~isempty(psf_params.scatter_sz))
      ...
      if(~isempty(scattvol))
        try
          Tidx1 = (1:zAsz(1))+size(scattvol,1)/2-zAsz(1)/2;
          Tidx2 = (1:zAsz(2))+size(scattvol,2)/2-zAsz(2)/2;
          if(rem(zA,proppx))
             neur_ves_A = cat(3,groupzproject(scattvol(Tidx1,Tidx2,1:zA-rem(zA,proppx)),proppx,'sum'), ...
                sum(scattvol(Tidx1,Tidx2,zA-rem(zA,proppx)+1:zA),3));
          else
             neur_ves_A = groupzproject(scattvol(Tidx1,Tidx2,1:zA),proppx,'sum');

          Tidx1 = (1:zBsz(1))+size(scattvol,1)/2-zBsz(1)/2;
          Tidx2 = (1:zBsz(2))+size(scattvol,2)/2-zBsz(2)/2;
          neur_ves_B = scattvol(Tidx1,Tidx2,zA+1:zB);
          Tidx1 = (1:zCsz(1))+size(scattvol,1)/2-zCsz(1)/2;
          Tidx2 = (1:zCsz(2))+size(scattvol,2)/2-zCsz(2)/2;
          if(rem(zC-zB,proppx))
             neur_ves_C = cat(3,groupzproject(scattvol(Tidx1,Tidx2,zB+1:zC-rem(zC-zB,proppx)),proppx,'sum'), ...
                sum(scattvol(:,:,zA-rem(zA,proppx)+1:zA),3));
          else
             neur_ves_C = groupzproject(scattvol(Tidx1,Tidx2,zB+1:zC),proppx,'sum');

        catch
          warning('scattering volume is not large enough for given parameters')
          Tidx1 = (zAsz(1)-size(scattvol,1))/2;
          Tidx2 = (zAsz(2)-size(scattvol,2))/2;
          if(rem(zA,proppx))
             neur_ves_A = cat(3,groupzproject(scattvol(:,:,1:zA-rem(zA,proppx)),proppx,'sum'), ...
                sum(scattvol(:,:,zA-rem(zA,proppx)+1:zA),3));
          else
             neur_ves_A = groupzproject(scattvol(:,:,1:zA),proppx,'sum');

          neur_ves_A = padarray(neur_ves_A,[Tidx1 Tidx2],0,'both');
          Tidx1 = (1:zBsz(1))+size(scattvol,1)/2-zBsz(1)/2;
          Tidx2 = (1:zBsz(2))+size(scattvol,2)/2-zBsz(2)/2;
          neur_ves_B = scattvol(Tidx1,Tidx2,zA+1:zB);
          Tidx1 = (zCsz(1)-size(scattvol,1))/2;
          Tidx2 = (zCsz(2)-size(scattvol,2))/2;
          if(rem(zC-zB,proppx))
             neur_ves_C = cat(3,groupzproject(scattvol(:,:,zB+1:zC-rem(zC-zB,proppx)),proppx,'sum'), ...
                sum(scattvol(:,:,zA-rem(zA,proppx)+1:zA),3));
          else
             neur_ves_C = groupzproject(scattvol(:,:,zB+1:zC),proppx,'sum');

          neur_ves_C = padarray(neur_ves_C,[Tidx1 Tidx2],0,'both');
        ...
        if(rem(zCsz(3),1)~=0)
          neur_ves_C(:,:,) = neur_ves_C(:,:,)*rem(zCsz(3),1);



      neur_ves_A2 = masked_3DGP_test(ceil(zAsz),bsxfun(@times,scatter_sz,[1 1 1/proppx]),proppx,0,[],[],scatter_wt);
      if(rem(zAsz(3),1)~=0)
        neur_ves_A2(:,:,1) = neur_ves_A2(:,:,1)*rem(zAsz(3),1);

      neur_ves_B2 = masked_3DGP_test(zBsz,scatter_sz,1,0,[],[],scatter_wt);
      neur_ves_C2 = masked_3DGP_test(ceil(zCsz),bsxfun(@times,scatter_sz,[1 1 1/proppx]),proppx,0,[],[],scatter_wt);
      if(rem(zCsz(3),1)~=0)
        neur_ves_C2(:,:,) = neur_ves_C2(:,:,)*rem(zCsz(3),1);
      ...
      scatter_params = check_psf_params([]);
      scatter_sz = scatter_params.scatter_sz;
      scatter_wt = scatter_params.scatter_wt;
      neur_ves_A2 = masked_3DGP_test(ceil(zAsz),bsxfun(@times,scatter_sz,[1 1 1/proppx]),proppx,0,[],[],scatter_wt);
      if(rem(zAsz(3),1)~=0)
        neur_ves_A2(:,:,1) = neur_ves_A2(:,:,1)*rem(zAsz(3),1);
  ...
  if (~isfield(vol_out,'neur_ves_all'))#||~isequal(size(vol_out.neur_ves_all),vres*vasc_sz)
    ...
    TMPves = groupzproject(single(vol_out.neur_ves(:,:,zB+1:)),proppx,'sum');
  ...
  elseif(isempty(TMPvasc))
    ...
    neur_ves_A = TMPves+neur_ves_A2.*(TMPves>0) + neur_ves_A.* ...
      bsxfun(@minus,ones(size(TMPves,1),size(TMPves,2),'single'),TMPves/proppx);

    TMPves = single(vol_out.neur_ves_all(floor((vascpx(1)-zBsz(1))/2)+(1:zBsz(1)), ...
      floor((vascpx(2)-zBsz(2))/2)+(1:zBsz(2)),zA+1:zB));
    neur_ves_B = TMPves+neur_ves_B2.*(TMPves>0) + neur_ves_B.* ...
      bsxfun(@minus,ones(size(TMPves,1),size(TMPves,2),'single'),TMPves);

    TMPves = groupzproject(single(vol_out.neur_ves_all(:,:,zB+1:)),proppx,'sum');
    if(size(TMPves,3)>size(neur_ves_C,3))
      neur_ves_C = TMPves(:,:,1:size(neur_ves_C,3))+neur_ves_C2.*(TMPves(:,:,1:size(neur_ves_C2,3)>0)) + neur_ves_C.* ...
    ...
    elseif(size(TMPves,3)==size(neur_ves_C,3))
      neur_ves_C = TMPves+neur_ves_C2.*(TMPves>0) + neur_ves_C.* ...
        bsxfun(@minus,ones(size(TMPves,1),size(TMPves,2),'single'),TMPves/proppx);
    else
      neur_ves_C(:,:,1:size(TMPves,3)) = TMPves+neur_ves_C2(:,:,1:size(TMPves,3)).*(TMPves>0) + neur_ves_C(:,:,1:size(TMPves,3)).* ...
        bsxfun(@minus,ones(size(TMPves,1),size(TMPves,2),'single'),TMPves/proppx);
    ...
    TMPves = groupzproject(single(TMPvasc(:,:,zB+1:)),proppx,'sum');
    if(size(TMPves,3)>size(neur_ves_C,3))
      neur_ves_C = TMPves(:,:,1:size(neur_ves_C,3))+neur_ves_C2.*(TMPves(:,:,1:size(neur_ves_C2,3)>0)) + neur_ves_C.* ...
        bsxfun(@minus,ones(size(TMPves,1),size(TMPves,2),'single'),TMPves(:,:,1:size(neur_ves_C,3))/proppx);
    elseif(size(TMPves,3)==size(neur_ves_C,3))
  ...
  if ~isstruct(Uin)
    if(psf_params.fastmask)
      ...
      elseif strcmp(psf_params.type,'vtwins')
        UinFM = generateVtwinsBA(vol_params,psf_paramsFM);
      elseif strcmp(psf_params.type,'bessel')
        UinFM = generateBesselBA(vol_params,psf_paramsFM);
      else
        warning('Needs to be a supported type, defaulting to ''gaussian''')
        UinFM = generateBA(vol_params,psf_paramsFM);
  ...
  #       psf_paramsFM.fineSamp = psf_params.FM.fineSamp;
  #
      [mask,~,~,psfTM,psfBM] = genCorticalLightPathLite(vol_params,psf_paramsFM, ...
                                          ves_A_FM,ves_B_FM,ves_C_FM,UinFM);        # Generate diffraction-limited tissue obscuration mask
      ...
      [~,psf,psfs,psfT,psfB] = genCorticalLightPathLite(vol_params,psf_params, ...
      ...
      [mask,psf,psfs,psfT,psfB] = genCorticalLightPathLite(vol_params,psf_params, ...
                                          neur_ves_A,neur_ves_B,neur_ves_C,Uin);        # Generate diffraction-limited tissue obscuration mask
      if(psf_params.sampling>max(size(mask)))
        mask       = ones(size(mask),'like',mask);
  ...
  if (~isfield(psf_params,'objNAcol'))&&(~isempty(psf_params.objNAcol))
  ...
  for i = 1:length(coldist)
    if(coldist(i)>0):
      se = autocrop(single(rho<=coldist(i).*(1./sqrt((rho.^2)/(coldist(i)^2)+1))));
      se = se/sum(se(:));

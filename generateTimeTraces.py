...
from check_spike_opts import check_spike_opts

def generateTimeTraces(spike_opts, varargin):
...
## Input Parsing
    ...
    spike_opts.nt = np.ceil(spike_opts.nt*100*spike_opts.dt)                     # Spiking is simulated at 100Hz, so make the time traces longer so that later downsampling gives the desired # outputs
    ...
    if ((hasattr(spike_opts,'spikeflag')) and spike_opts.spikeflag):               # Check whether or not to save the spiking process itself
        spikeflag = 1                                                         # Default to saving it
    else
        spikeflag = 0                                                         # Alternative is not to save it 


    if ((hasattr(spike_opts,'dendflag')) and not(spike_opts.dendflag)):                # Check the dendritic simulation flag (might not be set for nuclear imaging)
        dendflag = 0                                                          # If not set, don't do extra work
    else
        dendflag = 1                                                          # If set, also simulate dendrites

    if ((hasattr(spike_opts,'axonflag')) and not(spike_opts.axonflag)):                # Check the axonal simulation flag (might not be set for nuclear imaging)
        axonflag = 0                                                          # If not set, don't do extra work
    else
        axonflag = 1                                                          # If set, also simulate axons

    if ((spike_opts.N_bg>0)and(axonflag)):                                         # Check if both kind of background is set (axonal neuropil models or the older Gaussian Process neuropil models)
        raise ValueError('background must be either axons or GPs!')                       # Error out if both are set as it is not clear what to do
    ...
    if nargin > 1
        ...
        if (not(isempty(S_times))and((size(S_times,1) not(= spike_opts.K)||...
                                           (size(S_times,2) not(= spike_opts.nt)) # Check if there is a correctly sized pre-determined set of spiking activity for the cells
            warning('Incompatible size: Make sure number of components and time traces length is OK')
        end
        if (not(isempty(S_times))
            if(size(S_times,1) not(= spike_opts.K)
                spike_opts.K = size(S_times,1);
            end
            if(size(S_times,2) not(= spike_opts.nt)
    ...
    switch spike_opts.dyn_type
        ...
        case 'single'
            ...
            [not(,not(,S_somas] = calcium_dynamics(S_times, cal_params, prot);       # Simulate the dynamics and recover the fluoresence over time               
    ...
    #         cal_params.ext_rate = 292.3;                                       # Somas needs an even higher extrusion rate
            [not(,not(,S_somas]  = calcium_dynamics(S_times, cal_params, prot, 1, 1);# Simulate the dynamics and recover the fluoresence over time               
            S_somas        = S_somas(:,l_buff+1:end);                          # Remove buffering period
        case 'double'
    ...
    switch spike_opts.dyn_type
        ...
        case 'single'
            ...
            S_times       = (7.6e-6)*(S_times/min(S_times(S_times>0)));        # Normalize spike amplitudes to be reasonable calcium concentrations (in M)
            [not(,not(,S_dend]  = calcium_dynamics(S_times, cal_params, prot);       # Simulate the dynamics and recover the fluoresence over time               
            S_dend        = S_dend(:,l_buff+1:end);                            # Remove buffering period\
        case 'Ca_DE'
            cal_params.sat_type = 'Ca_DE';                                     # Set the calcium dynamics to 'single' mode (simpler dynamics)
            cal_params.dt       = 1/100;                                       # Set the calcium dynamics framerate
    #         cal_params.ext_rate = 292.3/2;                                     #
            [not(,not(,S_dend]  = calcium_dynamics(S_times, cal_params, prot, 1,1/2);# Simulate the dynamics and recover the fluoresence over time               
            S_dend        = S_dend(:,l_buff+1:end);                            # Remove buffering period
        case 'double'
            warning('The "double" method is not fully vetted: use at your own risk.')
            cal_params.sat_type = 'double';                                    # Set the calcium dynamics to 'double' mode (complex dynamics)
    ...
    switch spike_opts.dyn_type
        ...
        case 'single'
            ...
            cal_params.ext_rate = 400;
            S_times       = (7.6e-6)*(S_times/min(S_times(S_times>0)));        # Normalize spike amplitudes to be reasonable calcium concentrations (in M)
            [not(,not(,S_axon]  = calcium_dynamics(S_times, cal_params, prot);       # Simulate the dynamics and recover the fluoresence over time               
            S_axon        = S_axon(:,l_buff+1:end);                            # Remove buffering period\
        case 'Ca_DE'
            cal_params.sat_type = 'Ca_DE';                                     # Set the calcium dynamics to 'single' mode (simpler dynamics)
            cal_params.dt       = 1/100;                                       # Set the calcium dynamics framerate
    #         cal_params.ext_rate = 292.3/4;                                     # 
            [not(,not(,S_axon]  = calcium_dynamics(S_times, cal_params, prot, 1,1/4);# Simulate the dynamics and recover the fluoresence over time               
            S_axon        = S_axon(:,l_buff+1:end);                            # Remove buffering period
        case 'double'
            warning('The "double" method is not fully vetted: use at your own risk.')
            cal_params.sat_type = 'double';                                    # Set the calcium dynamics to 'double' mode (complex dynamics)
    ...
    if spike_opts.N_bg>0                                                       # If background activity is requested, calculate it
        bgscale      = 0.5;                                                    # Scale the background to not( 1/2 power
        ...
        switch spike_opts.dyn_type
            ...
            case 'single'
                ...
                [not(,not(,S_bg]    = calcium_dynamics(S_times, cal_params, prot);   # Simulate the dynamics and recover the fluoresence over time               
    ...
    if spike_opts.dt not(= 1/100                                                  # Only do this if the sampling rate is not 100Hz
        buff = 100;
    ...
    if(not(isempty(S.dend)); S.dend = bsxfun(@times, S.dend, mod_vals);  end      # Modulate the activity of each cell's dendrites

    if not(isempty(S.bg);    S.bg = bsxfun(@times, S.bg, mod_vals);               # Modulate the activity of each cell's dendrites
    else;                 S.bg = S.dend;
    end

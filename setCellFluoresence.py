...
#                       (default = 100um)
#       .drite_tau - Drite decay strength exponential distance
#                       (default = 5)
...
#   - d_params - Struct containing parameters for drite simulation
#       .dtParams    - dritic tree number,radius in um of branches
#                      (uniform across circle),radius in um of branches
#                      (uniform in z) (default = [40 150 50 1 10])
#       .atParams    - Apical drite number,radius in um of branches
#                      (uniform across circle),radius in um of branches
#                      (uniform in z),offset from center in um (default
#                      = [1 5 2 2 4]) 
#       .atParams2   - Through-volume apical drite number,radius in
...
#       .thicknessScale  - Scaling for drite thickness (int). Should be
...
#                      of drites at branching locations (default =
#                      1.5) 
...
#                   volume occupied by the k^th apical drite
...
#                   fluorescence (either soma, drite, vasculature, or
#                   background/neuropil) 
# 
...
#                base fluorescence (either soma, drite, vasculature, or
#                background/neuropil) 
#
...
## Input parsing
import numpy as np 
def setCellFluoresence(vol_params, neur_params, d_params, neur_num, neur_soma, neur_num_AD, neur_locs, neur_vol)
    ...
    d_params = check_d_params(d_params);                              # Check drite parameters
    ...
    gp_vals = dict(N_neur+vol_params.N_den,3);                                 # Initialize the array that will contain the locations of the cell's interior points and the local relative fluorescences
    wtSc    = d_params.weightScale;
    ...
    #     TMP_AD   = (vec(find(neur_num_AD==kk)));                               # Get all the points in the drites. Save memory by only storing the locations...
        [~,TMP_idxs,~]    = intersect(TMP_loc,TMP_soma);                       # Find where the soma and interior points intersect
        [~,TMP_idxs_AD,~] = intersect(TMP_loc,TMP_AD);                         # Find where the drites and interior points intersect
    ...
    #         (1-wtSc(3)*rand(size(gp_vals{kk,1})));                             # ... and the values
        ...
        gp_vals{kk,2}(TMP_idxs_AD) = ones(size(TMP_AD));                       # Store the values at the drites

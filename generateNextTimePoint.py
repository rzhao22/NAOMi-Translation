...
def generateNextTimePoint(spike_opts, cal_params, n_locs, mod_vals, resampStruct):
  if(spike_opts.smod_flag != 'hawkes'):
    print('Using hawkes process....')
  ###########################################################################
  ## Generate spike times

  spike_opts.dt = 1/100;

  if(len(resampStruct) == 0):
    resampStruct = dict;
  ...
  spikes       = np.single((7.6e-6)*spikes);                                # Normalize spike amplitudes to be reasonable calcium concentrations (in M)
  ###########################################################################
  ...
  return [resampStruct,spikes,fluor]

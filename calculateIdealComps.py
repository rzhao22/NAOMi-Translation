...
def calculateIdealComps(vol_out,PSF_struct,neur_act,scan_params,noise_params, spike_opts, tpm_params, num_comps)
    conds = fieldnames(neur_act);                                              # 
    if(isstruct(neur_act.(conds{1})))
        neur_base.soma = ones(size(neur_act.(conds{1}).soma,1));               # 
        neur_base.d = ones(size(neur_act.(conds{1}).d,1));               # 
        ...
        if nargin>5
            ...
            neur_base.d = neur_base.d(:,compIdxs);                       #    and drite time-traces
        ...
        if nargout > 1
            for i = 1:length(conds)
                ...
                f0.d       = min(neur_act.(conds{i}).d,[],2);            #   | - Create temporary "activity" with one time point - the minimum values
        ...
        n1 = min(neur_act.d,[],2);                                          # Get minimum value for each neuron's drite activity
        ...
        neur_base.d = zeros(size(neur_act.d,1),sum(gIdxs),'single');
        ...
        neur_base.d(sub2ind(size(neur_base.d),find(gIdxs)',1:sum(gIdxs))) = n1(gIdxs);
        ...
        if nargout > 1
            f0.soma = n0;                                                    # -----
            f0.d = n1;                                                    #   | - Create temporary "activity" with one time point - the minimum values
            f0.bg   = n2;                                                    # -----

...
#                  cell at the soma 
#    .d       - Kx(nt) matrix where each row is the activity of each 
#                  cell at the drites [can be optional]
...
def scan_volume_frame(scan_vol, neur_act, scan_params, z_off):
	...
	d_act = single(neur_act.d);
	d_act(d_act<cutoff) = 0;
	...
	# y_shr = cat(1,y_shr, y_shr()*ones(max(0,N1-numel(y_shr)),1));   # Make sure vector is the full size (N1)
	...
	for ll = 1:size(d_act,1)                                        # For large volumes, mex functions decrease runtimes a lot
	  if (d_act(ll)>0)&&(!isempty(scan_vol.dVol{ll,1}))
		array_SubSubTest(TMPvol,scan_vol.dVol{ll,1},...
		  scan_vol.dVol{ll,2},d_act(ll));
	...
	if(floor(sfrac)==sfrac)
		clean_img = conv2(clean_img, ones(sfrac,sfrac), 'same');           # Sum up nearby pixels
		clean_img = clean_img(1:sfrac:,1:sfrac:);                    # Subsample for actual sampling rate
	else
		clean_img = sfrac*sfrac*imresize(clean_img,1/sfrac,'bilinear'); # used for alternative scalings - values will be slightly off compared to local averaging
